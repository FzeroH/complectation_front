<template>
  <header v-if="userStore.hasUser" class="layout-header w-100 p-2 d-flex justify-content-between align-items-center">
    ...
    <nav>
      <ul class="layout-header__list d-flex">
		...
		<li class="layout-header__item p-2">
			<input type="button" class="layout-header__link fw-bold link-secondary" value="Выход" @click="logout">
		  </li>
      </ul>
    </nav>
  </header>
...
const teacherRoutes = computed<HeaderRoute[]>(() => (userStore.isTeacher ? [
	{ title: 'Список', to: '/teacher' },
	{ title: 'Профиль', to: '/profile' }
] : []))

const librarianRoutes = computed<HeaderRoute[]>(() => (userStore.isLibrarian ? [
