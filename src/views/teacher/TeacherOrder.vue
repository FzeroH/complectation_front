<template>
  ...
  <section v-else class="teacher-order">
    ...
    <div class="d-flex teacher-order__row mb-4">
      ...
      <custom-select
        :items="pub_types"
        :selected="pub_type_name"
        @change="pubTypeChange"
        title="Тип издания"
        class="ms-2"
      />
    </div>
    ...
    <table class="table table-light table-hover table-pointer w-100">
      <thead>
        <tr>
          ...
          <th scope="col" class="text-center">Кол-во студентов</th>
      ...
      <tbody>
        <tr v-for="(item, index) in recommendList" :key="index">
          <td>
            <custom-select
              :items="disciplines"
              :selected="item.discipline"
              title="Дисциплина"
              @change="recDisciplineChange(index, $event)"
            />
          </td>
          <td>
            <custom-select
              :items="student_groups[index]"
              :selected="item.students_group"
              title="Группа"
              @change="recStudentGroupChange(index, $event)"
              :disabled="!student_groups[index]?.length"
            />
          </td>
          <td class="text-center">{{ item.students_group_type || '-' }}</td>
          <td class="text-center">
            {{ item.students_discipline_semester === 0 ? '-' : item.students_discipline_semester }}
          </td>
          <td class="text-center">{{ item.students_group_count }}</td>
          <td class="text-center">
            <svg
              ...
              height="24"
              fill="currentColor"
              class="bi bi-trash3 text-danger pointer"
              viewBox="0 0 16 16"
        ...
        <tr>
          <td class="text-center" colspan="6">
            <svg
              ...
              height="32"
              fill="currentColor"
              class="bi bi-plus-square text-success pointer"
              viewBox="0 0 16 16"
            >
              <path
...
async function recStudentGroupChange(rowIndex: number, item: ListItem | null) {
  ...
  if (item === null) {
    ...
    const resRecOtherInfo = await TeacherApi.getRecOtherInfo(
      +recItem.discipline!.value!,
      +recItem.students_group!.value!,
    )
